...
@section('content_header')
    <h1>Lista metas secretaría general</h1>

    <div class="float-right">
        <a href="{{ route('pi-sec-generals.create') }}" class="btn btn-block btn-outline-secondary btn-sm float-right"  data-placement="left">
            {{ __('Nuevo') }}
        </a>
    </div>
    <div class="row">
        ...
        <tbody>
            @foreach ($pis->where('entidad_responsable', 'SECRETARÍA GENERAL') as $pi)
                <tr>
                    ...
                    <td>
                        <form action="{{ route('pi-sec-generals.destroy',$pi->id) }}" method="POST">
                            <a class="btn btn-sm btn-secundary" href="{{ route('pi-sec-generals.show',$pi->id) }}"><i class="fa fa-fw fa-eye"></i> {{ __('') }}</a>
                            <a class="btn btn-sm btn-secundary" href="{{ route('pi-sec-generals.edit',$pi->id) }}"><i class="fa fa-fw fa-edit"></i> {{ __('') }}</a>
                            @csrf
                            @method('DELETE')
