...
@section('content_header')
    <h1>Lista metas secretaría de gobierno</h1>

    <div class="float-right">
        <a href="{{ route('pi-sec-gobiernos.create') }}" class="btn btn-block btn-outline-secondary btn-sm float-right"  data-placement="left">
            {{ __('Nuevo') }}
        </a>
    </div>
    <div class="row">
...
@section('content')
    <table id="example" class="table table-striped table-bordered" style="width:100%">
        ...
        <tbody>
            @foreach ($pis->where('entidad_responsable', 'SECRETARÍA DE GOBIERNO') as $pi)
                <tr>
                    <td>{{ ++$i }}</td>
                    ...
                    <td>
                        <form action="{{ route('pi-sec-gobiernos.destroy',$pi->id) }}" method="POST">
                            <a class="btn btn-sm btn-secundary" href="{{ route('pi-sec-gobiernos.show',$pi->id) }}"><i class="fa fa-fw fa-eye"></i> {{ __('') }}</a>
                            <a class="btn btn-sm btn-secundary" href="{{ route('pi-sec-gobiernos.edit',$pi->id) }}"><i class="fa fa-fw fa-edit"></i> {{ __('') }}</a>
                            @csrf
                            @method('DELETE')
